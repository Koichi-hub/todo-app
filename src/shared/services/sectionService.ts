...
export const sectionService = {
    async getAll(): Promise<Section[]> {
        ...
        const rows = await db.select<any[]>('SELECT * FROM sections');
        return rows.map(mapRowToSection);
    },

    async getById(id: string): Promise<Section | undefined> {
        const db = await getDb();
        const rows = await db.select<any[]>('SELECT * FROM sections WHERE id = ?', [id]);
        return rows.length > 0 ? mapRowToSection(rows[0]) : undefined;
    },

    async getByProjectId(projectId: string): Promise<Section[]> {
        const db = await getDb();
        const rows = await db.select<any[]>('SELECT * FROM sections WHERE project_id = ?', [projectId]);
        return rows.map(mapRowToSection);
    },

    async getByTagId(tagId: string): Promise<Section[]> {
        const db = await getDb();
        const rows = await db.select<any[]>('SELECT * FROM sections WHERE tag_id = ?', [tagId]);
        return rows.map(mapRowToSection);
    },
